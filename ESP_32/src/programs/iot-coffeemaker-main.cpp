...
// NEU: neuer WiFi-Handler (Namespace)
#include "wifi/wifi_handler.h"

// dein existierender MQTT/NTP + Sensor-Kram
...
// MQTT:
static MqttHandler mqttHandler("IOT-Coffeemaker1");
static const IPAddress MQTT_BROKER_IP(192, 168, 178, 21); // fallback (BaristaBandwidth)
static const u16_t    MQTT_BROKER_PORT = 1883;

// Host/URL + Credentials (Primärpfad)
static const char* MQTT_HOST = "***";
static const char* MQTT_USER = "***";
static const char* MQTT_PWD  = "***";

// Publish
Stopwatch32MS publishWatch;
static const u32_t PUBLISH_INTERVAL_MILLIS = 200;
static const char* PUBLISH_TOPIC_COFFEE  = "Group-X/Coffemachine/data";
static const char* PUBLISH_TOPIC_CONTROL = "Group-X/Coffemachine/data";

// (Subscribe derzeit ungenutzt)
...
// Display/Touch (gelassen wie bei dir)
...
void setup(){
  ...
  cfg.tx_power_dbm = 8;  // Tx-Power wird NACH WiFi.begin gesetzt
  ...
  // -------- NTP (optional) --------
  if(!ntpHandler.begin(10000)) { ESP_LOGW(ESP_LOG_TAG, "NTP start failed"); }

  // -------- MQTT: zuerst Host/URL + Auth, sonst IP ohne Auth --------
  bool mqtt_ok = false;
  // Falls dein Handler die Host+Auth-Variante hat:
  mqtt_ok = mqttHandler.connect(MQTT_BROKER_IP, MQTT_BROKER_PORT, mqttCallback,3000);
  if (!mqtt_ok) {
    ESP_LOGW(ESP_LOG_TAG, "MQTT host connect failed, trying IP (no auth) …");
    mqtt_ok = mqttHandler.connect(MQTT_BROKER_IP, MQTT_BROKER_PORT, mqttCallback, 3000);
    // Hinweis: callback ist optional – Subscribe ist eh auskommentiert
  }
...
void loop() {
  ...
  if(!buttonLeftFlag && buttonCoffeeLeft.isPressed()){
    buttonLeftFlag = true;
    if(slider.getState() == SliderButton::State::Left)  jsonDocCoffee["label"] = "single small";
    else if(slider.getState() == SliderButton::State::Right) jsonDocCoffee["label"] = "single large";
  }
  if(!buttonCoffeeLeft.isPressed()) buttonLeftFlag = false;

  if(!buttonRightFlag && buttonCoffeeRight.isPressed()){
    buttonRightFlag = true;
    if(slider.getState() == SliderButton::State::Left)  jsonDocCoffee["label"] = "single large";
    else if(slider.getState() == SliderButton::State::Right) jsonDocCoffee["label"] = "double large";
  }
  if(!buttonCoffeeLeft.isPressed()) buttonRightFlag = false;
  ...
  if (running &&
      delayStopwatch.getTimeSinceStart() > 2000 &&
      sensorLightLeft.getState()  == LdrBlinkSensor::STATE::OFF &&
      sensorLightRight.getState() == LdrBlinkSensor::STATE::OFF) {
    running = false;
