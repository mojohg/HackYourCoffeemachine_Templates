...
ConnectResult connect(const WifiConfig& cfg, uint32_t timeout_ms) {
  ...
  if (cfg.scan_for_channel && channel == 0) {
    int ch = scanAndFindChannel(cfg.ssid);
    if (ch > 0) {
      channel = ch;
      ESP_LOGI(TAG, "Using scanned channel %d for SSID %s", ch, cfg.ssid);
    }
  }

  if (cfg.use_static_ip) {
  IPAddress dns1 = (cfg.dns1 != IPAddress(0,0,0,0)) ? cfg.dns1 : cfg.gateway;
  WiFi.config(cfg.local_ip, cfg.gateway, cfg.subnet, dns1, cfg.dns2);
}
